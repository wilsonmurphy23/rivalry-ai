...
const PlayersPage = ({ allPlayers }) => {
    ...
    return (
        <div className="pt-24 pb-24 px-4 sm:px-6 min-h-screen max-w-7xl mx-auto">
                ...
                <div className="animate-fade-in">
                    <div className="flex flex-col md:flex-row gap-3 mb-6">
                        {selectedTeam && (
                            <button onClick={() => { setSelectedTeam(null); setViewMode('teams'); }} className="bg-white/10 hover:bg-white/20 text-white px-4 py-3 rounded-xl font-bold flex items-center gap-2 smooth">
                                <Icon name="chevronDown" className="rotate-90" size={16} />Back
                            </button>
                        )}
                        <div className="relative flex-1">
                            <Icon name="search" size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                            <input type="text" placeholder={`Search ${selectedTeam || 'players'}...`} value={search} onChange={(e) => setSearch(e.target.value)} className="w-full glass rounded-xl pl-11 pr-4 py-3 text-white text-sm outline-none focus:border-purple-500 border border-transparent transition-all" />
                        </div>
                        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="glass rounded-xl px-4 py-3 text-white text-sm outline-none cursor-pointer hover:bg-white/5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredPlayers.slice(0, displayLimit).map(player => (
                            <div key={player.id} onClick={() => setSelectedPlayer(player)} className="glass-strong rounded-xl p-4 flex items-center justify-between cursor-pointer hover:bg-white/10 smooth border border-transparent hover:border-white/10 active:scale-[0.99]">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 flex items-center justify-center text-3xl">{player.image}</div>
                                    <div>
                                        ...
                                        <div className="text-gray-400 text-xs mt-1 flex items-center gap-2">
                                            <span className="bg-white/10 px-2 py-0.5 rounded text-gray-300">{player.position}</span>
                                            <span>{player.teams[0] || 'Free Agent'}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className={`text-xl font-black ${player.eraAdjustedVOR >= 90 ? 'text-yellow-400' : player.eraAdjustedVOR >= 80 ? 'text-purple-400' : 'text-gray-500'}`}>{player.eraAdjustedVOR}</div>
                                    <div className="text-[10px] text-gray-600 font-bold uppercase tracking-wider">VOR</div>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    {filteredPlayers.length > displayLimit && (
                        <button onClick={() => setDisplayLimit(prev => prev + 50)} className="w-full py-6 text-gray-500 font-bold hover:text-white smooth text-sm mt-4">Load More Players ({filteredPlayers.length - displayLimit} remaining)</button>
                    )}
                </div>
            )}
            ...
            {selectedPlayer && (
                <div
                    className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center sm:p-4 fade-in backdrop-blur-sm"
                ...
                >
                    ...
                    >
                        ...
                        <div className="p-6 bg-gradient-to-b from-white/5 to-transparent border-b border-white/10 text-center relative">
                            <button
                                onClick={() => setSelectedPlayer(null)}
                                className="absolute top-4 right-4 text-gray-400 hover:text-white p-2 z-50 hover:bg-white/10 rounded-full smooth"
                            >
                                <Icon name="x" size={24} />
                            </button>

                            <div className="text-6xl mb-4 filter drop-shadow-lg transform hover:scale-110 transition-transform duration-300">
                                {selectedPlayer.image}
                            </div>

                            <h2 className="text-2xl font-black text-white mb-1">{selectedPlayer.name}</h2>

                            <div className="flex justify-center gap-2 text-sm text-gray-300 mb-4">
                                <span>{selectedPlayer.teams[0]}</span>
                            ...
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-purple-500/20 border border-purple-500/30">
                                <span className="text-purple-300 font-bold text-sm">VOR Rating</span>
                                <span className="text-white font-black text-lg">{selectedPlayer.eraAdjustedVOR}</span>
                            </div>
                        </div>

                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                            <h3 className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-4">2025 Season Stats</h3>
                                ...
                                {Object.entries(selectedPlayer.stats).map(([key, value]) => {
                                    ...
                                    return (
                                        <div key={key} className="bg-white/5 rounded-xl p-3 border border-white/5 hover:border-white/10 transition-colors">
                                            <div className="text-gray-500 text-[10px] uppercase tracking-wider mb-1 truncate">{formatKey(key)}</div>
