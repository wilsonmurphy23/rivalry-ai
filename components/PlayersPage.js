/* ====================================
   PLAYERS PAGE (REDESIGNED: CLEAN UI)
   ==================================== */

const PlayersPage = ({ allPlayers }) => {
    const [sport, setSport] = React.useState('NBA'); // 'NBA' or 'NFL'
    const [viewMode, setViewMode] = React.useState('list'); // 'list' or 'teams'
    const [selectedTeam, setSelectedTeam] = React.useState(null);

    const [search, setSearch] = React.useState('');
    const [sortBy, setSortBy] = React.useState('vor'); // 'vor', 'name'
    ...
    // Helper to format stats keys
    const formatKey = (key) => key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());

    // Helper to reset filters when switching sports
    ...
    return (
        <div className="pt-24 pb-24 px-4 sm:px-6 min-h-screen max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">
                        {selectedTeam ? `Viewing ${selectedTeam} Roster` : `Browsing ${sportPlayers.length} Players`}
                    </p>
                </div>
            ...
            {/* === VIEW 2: PLAYER LIST (Grid Layout) === */}
            {(viewMode === 'list' || selectedTeam) && (
                <div className="animate-fade-in">

                    {/* Active Filters Bar */}
                    <div className="flex flex-col md:flex-row gap-3 mb-6">
                        {/* Back Button (If Team Selected) */}
                        {selectedTeam && (
                            <button
                        ...
                        <div className="relative flex-1">
                            ...
                            <input
                                type="text"
                                placeholder={`Search ${selectedTeam || 'all players'}...`}
                                value={search}
                    ...
                    {/* PLAYERS GRID (2 Columns on Desktop) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredPlayers.slice(0, displayLimit).map(player => (
                            <div
                                key={player.id}
                                onClick={() => setSelectedPlayer(player)}
                                className="glass-strong rounded-xl p-4 flex items-center justify-between cursor-pointer hover:bg-white/10 smooth border border-transparent hover:border-white/10 active:scale-[0.99]"
                            >
                                <div className="flex items-center gap-4">
                                    {/* ✅ FIXED: Removed black circle background */}
                                ...
                                <div className="text-right">
                                    <div className={`text-xl font-black ${player.eraAdjustedVOR >= 90 ? 'text-yellow-400' :
                                        player.eraAdjustedVOR >= 80 ? 'text-purple-400' :
                                            'text-gray-500'
                                        }`}>
                                        {player.eraAdjustedVOR}
                                    </div>
            ...
            {selectedPlayer && (
                ...
                >
                    <div
                        className="glass-strong w-full sm:max-w-lg sm:rounded-3xl rounded-t-3xl p-0 h-[85vh] flex flex-col slide-up overflow-hidden shadow-2xl"
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {Object.entries(selectedPlayer.stats).map(([key, value]) => {
                                    if (value == 0 || value == "0.0") return null; // Hide zero stats
