/* ====================================
   CREATE PAGE (FIXED: NO CROSS-SPORT)
   ==================================== */
...
const CreatePage = ({
    ...
    // ✅ FIXED: RESET EVERYTHING ON SPORT SWITCH
    const switchSport = (newSport) => {
        ...
        // Reset filters to avoid bugs
        setPositionFilter('all');

        // Prevent cross-sport comparison
    ...
    return (
        <div className="pt-24 pb-24 px-4 sm:px-6 min-h-screen max-w-7xl mx-auto">
            ...
            {selectedPlayer1 && selectedPlayer2 && (<button onClick={createMatchup} disabled={isSubmitting} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl py-4 text-white font-black text-lg smooth hover-scale disabled:opacity-50 mb-8 shadow-lg">{isSubmitting ? 'Creating Debate...' : '⚔️ Launch Debate'}</button>)}
